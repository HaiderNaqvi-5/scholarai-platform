...
| S89.3 | **Emil ui-primitive polish** (2026-05-18, branch `feat/s89-premium-cultural`, commit `1757420`): Emil Kowalski design-engineering pass on 5 core primitives in `src/components/ui/`. Button + IconButton: `transition-all` swapped for explicit prop list (`background-color,color,border-color,transform,box-shadow,opacity`); universal `active:scale(0.97)` / `active:scale(0.96)` press feedback with `will-change-transform` + `active:duration-75` snap-back; `disabled:active:scale-100` guard. Button loading spinner sped 1000ms → 700ms (`[animation-duration:700ms]`) for perceived perf. Dialog: overlay + content swap `data-[state] animate-in` keyframes for interruptible CSS transitions on `opacity` + `transform`; content enters from `scale(0.96)` w/ `origin-center` (skill modal exception — modals stay center-origin); exit uses `--motion-exit` (140ms) vs `--motion-enter` (180ms) for asymmetric feel; close button promoted to 44×44 `tap-target` with `active:scale(0.94)` and focus ring. Select: trigger `active:scale(0.99)` + explicit transition; content adds `origin-[var(--radix-select-content-transform-origin)]` so popover scales from trigger (not center) per Emil's origin-awareness rule + same interruptible opacity/scale pattern. Tabs trigger: explicit `transition-[color,background-color,box-shadow,transform]` w/ `var(--motion-micro)` + `var(--ease-out)` (was bare `transition-colors` default ease); `active:scale(0.98)` press; `data-[state=active]:shadow-[var(--shadow-hairline)]` for depth on active tab. Lint + tsc --noEmit + build green (40 routes); no new dependencies. | **Done** |
