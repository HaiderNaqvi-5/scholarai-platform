...
| S89.2 | **Landing rotator + IP-currency + stripe consistency** (2026-05-18, branch `feat/s89-premium-cultural`, commits `02bc5bb`, `7b85c46`): hero h1 cycles `bachelor's → master's → PhD` via new `components/marketing/RotatingDegree.tsx` (opacity-only fade, 2.2s hold + 320ms cross-fade, `aria-live="polite"`, respects `prefers-reduced-motion`, `min-w-[5ch]` reserves layout). `/upgrade` 5-button currency switcher deleted; replaced with `DetectedCurrency` chip driven by new `lib/geo/useGeoCurrency.ts` hook (ipwho.is, 24h localStorage cache `aidwise.geo_currency`, PKR fallback). Resolution precedence: `?currency=` query > `auth.user.plan_currency` > geo > PKR; error-state "Show in PKR" button preserved via internal `fallbackOverride` state. `ListEditor` in `(mentor)/mentor/documents/[id]` swapped raw `border-l-2 border-l-danger pl-3` for the `danger-stripe` `@utility` so all three tones share stripe geometry. Lint + tsc + build green (40 routes). Source-tree audits clean: 0 emoji / 0 gradient text / 0 heavy backdrop-blur / 0 heavy shadow-or-ring across 123 files. | **Done** |
...
| S94 | **Frontend design pass: audit P0/P1 + competitor features** (2026-05-29, branch `s94/frontend-design-pass`). Plan `~/.claude/plans/synthetic-popping-avalanche.md`. Phase 1 P0 (prior session, verified): mobile nav drawer (`components/shell/MobileNav.tsx` Radix left drawer + `GlobalAuthNav.tsx`), login split, onboarding "Other"+GPA preview, admin type-drift, `devIndicators:false`, em-dash sweep. Phase 2 competitor features: outcome CTAs ("See my matches" / "Show scholarships I qualify for"); consultant-vs-AidwiseAI comparison table before pricing; **live scrollable scholarships rail** `components/marketing/ScholarshipsRail.tsx` (real `scholarships.listPublic()` data, scroll-snap, edge-fade mask, no autoplay). Phase 3 P1 (8): Sidebar Matches icon Sparkles→Target; `RotatingDegree` min-w 10ch + reduced-motion static + blur-bridge; `Button` spinner absolute-overlay (no label jolt); `TopBar` alert-bell persistent for admins (no poll shift); password meter keystroke-gated + scaleX fill + criteria pills; **role-mixing fix** `primaryGroup(role)` in `RoleGuard.tsx` (admin/owner see only Admin nav); banner enter motion `slide-in-top`/`slide-in-bottom` utils (OfflineBanner/CookieBanner/ConsentBar); Elite pricing-cell drench (`PricingTeaser` `drench` prop). Type fixes: `RoleChangeAudit`→`AccessControlRoleChangeItem` (`previous_role`/`next_role`/`is_reversible`), new lean `ScholarshipListItem`/`ScholarshipListItemResponse`. Phase 4 motion polish: scroll-driven progress bar (progressive, reduced-motion-gated), CTA trailing-arrow nudge (`@media hover`), search `/`-hint + sidebar-kbd opacity fades. tsc+lint+build green (41 routes), emoji audit clean. Commits `c602006` (P1/P2 features) + `58d41e4` (Phase 4 motion). **Flagged pre-existing bug, DECISION=enrich backend list:** `/discover`+`ScholarshipCard` read `s.id`/`s.field_tags` but `GET /scholarships` returns lean `scholarship_id`/`provider_name`/`deadline_at` → /discover crashes on live data (backend ticket: enrich `ScholarshipListItem`; rail uses own typed `listPublic()`). Remaining P2 nits: skeleton→content crossfade, FAQ topic chips, footer refresh-chip. Plan's "2px lapis active rail" skipped (impeccable side-stripe ban; kept `bg-lapis-soft` fill). | **Done (core)** |

| S95 | **Wave 2: tasteful autoplay rail + deferred polish + audit extras** (2026-05-29 → 30, branch `s95/frontend-design-pass-wave2` off `s94/frontend-design-pass` tip rebased onto `s93/auth-tier-1` head `ad14fef`). Plan `~/.claude/plans/synthetic-popping-avalanche.md` (repurposed). Phase A: `ScholarshipsRail.tsx` autoplay — 5s interval + pause on hover/focus/IntersectionObserver/user-scroll-debounce + reduced-motion hard-stop via `useSyncExternalStore` + always-visible prev/next chevron arrows (44×44, aria-labels); WCAG 2.2.2 compliant + brand-ban override recorded as user-approved exception. Phase B: new `content-fade-in` `@utility` in `globals.css` applied to rail items, `/discover` results, `/feed` recs grid, `/admin` stat grid (opacity-only; reduced-motion-safe via global cap). Phase C: FAQ items gain `topic` field (Pricing/Vs consultants/Coverage/Accuracy/Privacy/Billing) rendered as `Badge` chip inside each `<summary>`. Phase D: footer "Updated" line reframed as a pill chip; `LAST_UPDATED_ISO/LABEL` constants dedupe the hero/footer dates (kills 2026-05-17/05-18 drift). Phase E1: `Button` spinner gated `motion-reduce:hidden`; label wrapper gated `motion-safe:invisible` → reduced-motion users no longer see a frozen spinner. Re-audit (3 parallel skill-lens Explore agents) findings folded in or flagged in plan. **Browser-verified live (chromium + bundled backend)**: 18 cards from live API, autoplay 0→272, hover-pause 272→272, Next click 272→568, reduced-motion 0→0 static, 0 console errors in both contexts. tsc+lint+build green (41 routes), emoji audit clean. Commit `f92b1e1`. **Deferred (separable)**: E2 banner exit motion for `OfflineBanner` / `CookieBanner` / `ConsentBar`. Pre-existing `/discover`+`ScholarshipCard` lean-list contract drift remains the **backend list-enrichment ticket** (S94 decision; rail untouched by it). | **Done (core)** |

## UI primitives (post-S88)

- `ui/button` (CVA, asChild Slot, `loading` spinner is absolute-overlay + `invisible` label wrapper → no width/label shift, sizes sm/md/lg/icon, variants: `primary` / `lapis` / `secondary` / `ghost` / `danger` / `gold` / `validated` / `link`).
...
- `marketing/RotatingDegree` — post-S89.2, hero h1 word cycler (bachelor's → master's → PhD, opacity fade, reduced-motion safe).

## Geo / locale hooks (post-S89.2)

- `lib/geo/useGeoCurrency.ts` — client hook. Fetches `https://ipwho.is/?fields=success,country_code,currency` once per 24h, caches `{currency, country, ts}` in `localStorage[aidwise.geo_currency]`. Maps `currency.code` to the supported set (PKR/GBP/EUR/AED/USD); falls back via `defaultCurrencyForCountry(country_code)` then PKR. Aborts on unmount. Used by `/upgrade` (auto currency, no UI switcher).
