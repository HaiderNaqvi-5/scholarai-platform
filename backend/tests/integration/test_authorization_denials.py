...
def test_access_control_users_returns_403_without_owner_control_capability(app, client):
    async def override_current_user():
        return _fake_user(capabilities={"owner.system.read"})
    ...
    response = client.get("/api/v1/access-control/users", headers={"Authorization": "Bearer fake"})
