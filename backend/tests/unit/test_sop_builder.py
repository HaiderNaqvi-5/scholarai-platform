...
class _FakeDB:
    def __init__(self, existing_sops: int = 0):
        self._queue = [_FakeResult(existing_sops)]
        self.added: list[Any] = []
        self.flushed = 0
...
async def test_free_user_blocked_after_one_sop():
    db = _FakeDB(existing_sops=1)
    svc = SOPBuilderService(db)
    with pytest.raises(HTTPException) as excinfo:
        await svc.draft(_user("free"), _payload())
    err = excinfo.value
    assert err.status_code == 402
    assert err.detail["error"] == "plan_limit_reached"
    assert err.detail["existing_sops"] == 1


@pytest.mark.asyncio
async def test_free_user_allowed_first_sop():
    db = _FakeDB(existing_sops=0)
    svc = SOPBuilderService(db)
    response = await svc.draft(_user("free"), _payload())
...
@pytest.mark.asyncio
async def test_pro_user_bypasses_sop_cap():
    db = _FakeDB(existing_sops=5)
    svc = SOPBuilderService(db)
