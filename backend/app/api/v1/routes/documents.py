...
# P2-15: mirrors DocumentSubmissionValidation.scholarship_ids max_length in
# app/schemas/documents.py (kept as a plain constant here — that schema is
# built manually in the service layer, not via FastAPI's request-body
# validation, so a bad value there surfaces as a 500, not a 422; this
# route-level check rejects early with a proper 422 instead).
MAX_SUBMISSION_SCHOLARSHIP_IDS = 3
...
async def create_document(
    current_user: DocumentCreateUser,
    db: Annotated[AsyncSession, Depends(get_db)],
    document_type: Annotated[str, Form(..., max_length=64)],
    title: Annotated[str | None, Form(max_length=255)] = None,
    content_text: Annotated[str | None, Form(max_length=MAX_TEXT_LENGTH)] = None,
