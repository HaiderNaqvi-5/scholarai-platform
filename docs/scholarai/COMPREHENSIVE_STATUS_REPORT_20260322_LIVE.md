...
## 1) v0.1 SLC Status: done vs partial vs not done
...
## 2) Deferred research (explicitly treated as non-v0.1 SLC)

Based on research/roadmap docs, these remain deferred or exploratory tracks rather than v0.1 SLC blockers:
...
The following are documented as post-v0.1 SLC/startup-scale direction and are not fully implemented yet:
...
- Growth and monetization readiness tracks (commercial/scale operations beyond v0.1 SLC baseline).
...
- Status docs that mark v0.1 SLC as "not fully complete" align with current repo reality.
...
- ScholarAI is in a late-v0.1 SLC hardening and migration stage, not a fully finished startup-scale state.
