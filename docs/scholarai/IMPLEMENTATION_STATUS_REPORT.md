...
| Ingestion pipeline | Source-registry run model, capture path (Playwright→httpx fallback, conditional GET via ETag/Last-Modified), feed/sitemap discovery, `DiscoveryService` for LLM-classified source candidates, parsing across anchor/table/JSON-LD/**microdata** with **structured deadline + funding amount/currency extraction**, raw-record creation, worker hook with **`auto`-mode threshold routing**, **`run_nightly_ingestion` catch-up gating**, run retry API/UI controls, diagnostics-filtered run history (now including `parser_diagnostic`), queue assignment, bulk retry operations, captured HTML snapshot management with **drift detection that flags published records for revalidation**, **fuzzy-title dedup advisories**, **pagination across `rel=next` / numbered / Load More**, **per-source health monitoring (`GET /curation/source-health`)**, and **provenance trace API (`GET /scholarships/{id}/provenance`)**. Pakistan-first geo defaults with destination-aware country inference. | Parser coverage stays heuristic — broader provider-specific tuning, deeper resilience on non-standard listings, and authenticated/JS-heavy sources remain incomplete |
...
| Richer ingestion review controls | Run retry, parser-diagnostic filtering, queue assignment, bulk retry, captured HTML snapshot management, fuzzy-title dedup advisories, snapshot drift detection, source-health monitoring (`GET /curation/source-health`), and provenance trace (`GET /scholarships/{scholarship_id}/provenance`) are now all implemented (migration `20260514_0021`). Remaining: provider-specific parser tuning and authenticated/JS-heavy source coverage. |
