...
## Purpose
This document defines the v0.1 scholarship ingestion, validation, provenance, review, and publication workflow. It ensures that user-facing scholarship data is traceable, manually reviewable, and safe for eligibility and recommendation use.
...
## Source Registry
### v0.1 source classes
| Source class | Examples | Allowed in v0.1 | Trust level |
|---|---|---|---|
...
| DAAD pages | DAAD official scholarship pages | No for v0.1 publication | Deferred |
...
| Country scope allowed | Prevent non-v0.1 geography leakage |
| Degree level normalization | Enforce MS-only v0.1 scope |
...
## v0.1 decision
ScholarAI v0.1 will run a curator-governed pipeline in which raw records are parsed, validated, deduplicated, manually reviewed, and published only after explicit human approval.
...
## Assumptions
- The v0.1 team can maintain a protected internal curator workflow.
- Approved reference pages may help discovery but will not replace official publication authority.
- Nightly or event-driven refresh is sufficient for scholarship freshness in the v0.1 corpus.
