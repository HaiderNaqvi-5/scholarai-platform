...
- **HIBP password breach check** — `backend/app/core/hibp.py` (`is_pwned`, k-anonymity range query, fail-open) + `test_hibp.py` + `HIBP_TIMEOUT_SECONDS` config exist but are **untracked and UNWIRED** (zero callers; not in registration flow). Either wire into signup or drop.
