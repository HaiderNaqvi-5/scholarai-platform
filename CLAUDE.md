...
## Q1 Retier — Task 11 (2026-05-16, commit `e5ffe88`)
Burn-cap-aware LLM wrapper landed: `AnthropicClient.complete_with_accounting` pre-flights `assert_within_burn_cap` (HTTP 429 on budget breach) and post-records `record_llm` with the SDK's real usage tokens; offline / deterministic-template path writes a synthetic zero-cost ledger row keyed on the input-token estimate so the audit trail stays consistent in CI. Migrated callers: `sop_builder.draft` (endpoints `documents.sop.draft` + `documents.sop.line_feedback`), `visa_interview/evaluator.evaluate_answer` (now async — `db`/`user` threaded through `service.answer`). `professor_email` + `strategy_report` callers migrated in-tree (await landing of S87 WIP). `strategy_report._match_scholarships` switched to `ScholarshipMatchService.match_internal()` since the public `match()` response strips internal eligible/partial/stretch buckets (Task 7). New test `test_burn_ledger_records.py` covers both offline-fallback and stub-SDK accounting paths. 431 backend tests pass.

## Q1 Retier — Task 13 (2026-05-16, premium tier paywall on public catalog)
`backend/app/api/v1/routes/scholarships.py` — list endpoint adds `Scholarship.tier == ScholarshipTier.STANDARD` filter when caller is anonymous OR `can_see_premium(user)` is false. Detail + provenance endpoints route premium-tier rows through `_guard_premium_tier`: anonymous → 402 (generic upgrade payload, no `plan_currency` dereference), free → 402 via `raise_plan_required(["pro", "elite"], …)`, Pro / Elite / Institution → 200. New local `get_optional_user` dependency wraps `get_current_user` and swallows auth failures so the public surface stays anonymous-friendly while still threading the User row through plan checks when a valid bearer is presented. Tests in `backend/tests/integration/test_public_scholarships.py` add 6 cases (anon/free catalog excludes premium, pro catalog includes both, premium detail 402-for-free + 200-for-pro, standard detail open to all) via `dependency_overrides[get_optional_user]`. **438 backend tests pass.**
